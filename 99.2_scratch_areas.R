...
stehman <- read_csv(file="validation/data/test/stehman_test_data.csv")
...
#' @param map_label Vector with numeric codes representing the map labels, for
#' each sample.
...
#' described below.
#' @param totarea_pix Integer with the total number of pixels present in the 
#' original stratification map. 
...
#' reference class.
#' @param pixel Pixel size in square meters; for landsat, this is 30. 
...
calc_accuracies = function(strata_totals, sample_totals, rfcodes, totarea_pix,
	...
	return(list(overall_acc = overall_acc*100, overall_acc_min = overall_acc_min, 
							overall_acc_max = overall_acc_max, users_acc = users_acc, 
							users_acc_min = users_acc_min, users_acc_max= users_acc_min,
