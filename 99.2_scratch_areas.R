...
rfcodes <- c('A', 'B', 'C', 'D')
propAndVars <- calc_props_and_vars(orig_strata, ref_label, map_label, 
										strata_totals, sample_totals, rfcodes)
...
ref_var <- propAndVars$ref_var

propSE <- calc_se_prop(strata_totals, sample_totals, ref_var, 
											 rfcodes, totarea_pix)
...
calc_unbiased_area = function(totarea_pix, class_prop, se, pixel){
	# Total area in ha
...
#' A little test with data from Stehman, 2014.  
#+ TestBlock3
class_prop <- propAndVars$class_prop
pixel <- 30

unArea <- calc_unbiased_area(totarea_pix, class_prop, propSE, pixel)

#' ###calc_accuracies  
#' Function to calculate accuracies and their 95% confidence intervals.  
#'
#' **strata**_totals Dataframe with two columns and number of rows equal to 
#' the total number of classes in the original strata. The first column must 
#' have the same codes found in the original stratification and the second must 
#' have the total number of PIXELS of each class in that original strata map.  
#' **sample**_totals Dataframe with two columns and number of rows equal to 
#' the total number of classes in the original strata. The first column must 
#' have the same codes found in the original stratification, and the second must
#' have the total number of SAMPLE UNITS of each class collected from that 
#' original strata map.  
...
calc_accuracies = function(strata_totals, sample_totals, rfcodes, totarea_pix,
...
ref_props <- propAndVars$ref_prop
ref_vars <- propAndVars$ref_var
map_props <- propAndVars$map_prop
map_vars <- propAndVars$map_var
map_and_ref_props <- propAndVars$map_and_ref_prop
map_and_ref_vars <- propAndVars$map_and_ref_var
over_acc_props <- propAndVars$overall_acc_prop
over_acc_vars <- propAndVars$overall_acc_var
users_covs <- propAndVars$users_cov
prod_covs <- propAndVars$producers_cov

accurates <- calc_accuracies(strata_totals, sample_totals, rfcodes, totarea_pix,
									ref_props, ref_vars, map_props, map_vars, map_and_ref_props, 
									map_and_ref_vars, over_acc_props, over_acc_vars,
									users_covs, prod_covs)
