...
The following inputs must be contained in a list of data frames that are input as the parameter **props\_and\_vars**. They are produced by `calcPropsAndVars()` function. **ref\_prop** Dataframe with proportions (mean) of reference labels (columns) present on each sample strata class (rows).
